...
function addClassToCell(cell, elem, showType) {
  //add the right class to the cell based on its state
  let stateClass;
...
const timer = (milliseconds) => {
  return new Promise((toCall) => {
    setTimeout(toCall, milliseconds);
  });
};
function AI() {
  ...
  const placeRandom = function (AIgb, shipLength) {
    ...
    while (flag == false) {
      ...
      flag = !!AIgb.placeShip(x, y, axis, shipLength);
      console.log("PROBLEM=====");
    }
  };
...
function game(currP, nextP) {
  ...
  nextP.gb.update().forEach((column, x) =>
  ...
  function addEventHitCell(elem, x, y) {
    //add the handler for the hit to the element
    elem.addEventListener("click", (e) => {
      if (nextP.gb.receiveDamage(x, y)) {
...
function shipSetter(player) {
  const allShips = window.allShips || [
    { length: 3 },
    { length: 3 },
    { length: 3 },
  ];
...
function addEventPlaceShipToCells(anch, player, ships, axis) {
  ...
  player.gb.update().forEach((column, x) =>
    column.forEach((cell, y) => {
